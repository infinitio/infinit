...
namespace surface
{
  namespace gap
  {
                           ...
                           bool auto_add)
    {
      ...
      this->_reporter[response.created_network_id].store(
    ...
    NetworkManager::prepare(std::string const& network_id)
    {
      ...
      if (!elle::os::path::exists(description_filename))
      {
        ...
        if (this->one(network_id).descriptor.empty())
        {
          ...
          auto nb = operation_detail::blocks::create(network_id, identity);
                            ...
                            bool remove_directory)
    {
      ...
      {
        ELLE_TRACE("stopping infinit instance for network %s", network_id)
          this->_infinit_instance_manager.stop(network_id);
      }

      if (this->_networks->find(network_id) != this->_networks->end())
      {
        this->_reporter[network_id].store(
          "network.delete.attempt",
        ...
        ELLE_TRACE("remove network %s from meta", network_id)
          ...
          CATCH_FAILURE_TO_METRICS(network_id, "network.delete");
        this->_reporter[network_id].store(
          "network.delete.succeed",
          {{MKey::value, network_id}});
      }
                             ...
                             std::string const& peer_public_key)
    {
      ...
      CATCH_FAILURE_TO_METRICS(network_id, "network.adduser");

      this->_reporter[network_id].store(
        "network.adduser.succeed",
        {{MKey::value, network_id}});
    }
